...
alter function vehicles_report(date, date) owner to postgres;

create function vehicles_report(driver_registration_code character varying)
    returns TABLE(registration_code character varying, vin character varying, model_name character varying, brand_name character varying, manufacture_year integer, body_color character varying, category_code character varying, full_name character varying, date_of_registration date)
    language sql
as
$$
...
alter function vehicles_report(varchar) owner to postgres;

create function violations_report(start_date date, end_date date)
    returns TABLE(registration_code character varying, description text, fine_amount numeric, date_of_violation date, date_of_payment date)
    language sql
as
$$
	
	select * from violations_info
	where date_of_violation >= start_date and date_of_payment <= end_date
    order by date_of_violation desc;
$$;

alter function violations_report(date, date) owner to postgres;

create function violations_report(driver_registration_code character varying)
    returns TABLE(registration_code character varying, description text, fine_amount numeric, date_of_violation date, date_of_payment date)
    language sql
as
$$
...
alter function violations_report(varchar) owner to postgres;

create function categories_report()
    returns TABLE(category_code character varying, permitted_vehicle_name character varying, minimal_age integer)
    language sql
as
$$
...
alter function categories_report() owner to postgres;

create function driver_licenses_report(start_date date, end_date date)
    returns TABLE(full_name character varying, date_of_birth date, registration_address character varying, department_type character varying, date_of_issue date, date_of_expiry date)
    language sql
as
$$
...
alter function driver_licenses_report(date, date) owner to postgres;
